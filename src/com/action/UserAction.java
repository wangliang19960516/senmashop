...
public class UserAction extends PrimaryAction{
	...
	public String register(){
		if (user.getUsername().isEmpty()) {
			...
			return "register";
				...
				addActionMessage("电话不能为空!");
				return "register";
		}else if (user.getAddress().isEmpty()) {
			addActionMessage("地址不能为空!");
			return "register";
		}else if (user.getMail().isEmpty()) {
			addActionMessage("邮箱不能为空!");
			return "register";
		}else if (user.getPayword().isEmpty()) {
			addActionMessage("支付密码不能为空!");
			return "register";
	...
	public String pay() {
		...
		if(paytype==3) { 
			...
			if(Objects.nonNull(user) && user.getPayword().equals(u.getPayword())){
				indentService.pay(indentid, paytype);
	...
	public String my(){
		...
		if(type == 101) { // 修改个人信息
			...
			Users u = userService.get(user.getId());
			if(user.getPayword().equals(u.getPayword())){
				u.setPayword(user.getPasswordNew());
				userService.update(u);  // 更新数据库
